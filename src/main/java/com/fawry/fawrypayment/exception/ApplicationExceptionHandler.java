...
import com.fawry.fawrypayment.dto.ErrorResponse;
...
public class ApplicationExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleConstraintViolation(DataIntegrityViolationException ex) {
        ...
        ErrorResponse error=ErrorResponse.builder().errorMessage(message).status(HttpStatus.BAD_REQUEST.toString()).time(LocalDateTime.now()).build();
        return ResponseEntity
                .status(HttpStatus.BAD_REQUEST)
                .body(error);
    }
    ...
    public ResponseEntity<ErrorResponse> handleApplicationExcepion(ApplicationException ex){
        ...
        ErrorResponse error=ErrorResponse.builder().errorMessage(ex.getMessage()).status(ex.getCode().toString()).time(LocalDateTime.now()).build();
        return ResponseEntity
                .status(ex.getCode())
                .body(error);
    }
    ...
    public ResponseEntity<ErrorResponse> handleUnCategorizedExceptions(Exception ex){
        ...
        ErrorResponse error=ErrorResponse.builder().errorMessage(message).status(HttpStatus.INTERNAL_SERVER_ERROR.toString()).time(LocalDateTime.now()).build();
        return ResponseEntity
