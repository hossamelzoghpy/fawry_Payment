  ...
  template: `
    <div class="fixed top-4 right-4 z-50 space-y-3">
      @for (toast of toastService.toasts(); track toast.id) {
        <div 
          class="flex items-start gap-3 min-w-80 max-w-md p-4 rounded-xl shadow-2xl animate-slide-in"
          ...
          <div class="flex-1">
            <p class="text-sm font-semibold">{{ getToastTitle(toast.type) }}</p>
            <p class="text-sm mt-1">{{ toast.message }}</p>
          </div>

         
          <button 
            (click)="toastService.removeToast(toast.id); $event.stopPropagation()"
